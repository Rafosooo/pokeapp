...
export function getTypeEffectiveness(attacker: PokemonType, defenderTypes: PokemonType[]): number {
  let multiplier = 1;
  for (const defenderType of defenderTypes) {
    if (TYPE_CHART[attacker] && TYPE_CHART[attacker][defenderType] !== undefined) {
      multiplier *= TYPE_CHART[attacker][defenderType];
    }
  }
  return multiplier;
}
